...
%type <readin_statement> READIN_S
...
%destructor {free_readin_statement($$);} <readin_statement>
...
SELECT_S    : SELECT S_FIELDS FROM IDENTIFIER                   { $$ = new_select_statement($2,$4,NULL) }
	    ...
	    ;
READIN_S    : READIN IDENTIFIER					{ $$ = new_readin_statement($2)}
	    ;
...
STATEMENT   : SELECT_S ';'				        { returned_statement = new_statement(); $$=returned_statement; returned_statement->select=$1;}
	    ...
	    | READIN_S ';'					{ returned_statement = new_statement(); $$=returned_statement; returned_statement->rf=$1;}
	    ;
%%
